...
export const InputNumber: React.FC<InputNumberProps> = ({
  ...
  const getWrapperClass = () => {
    ...
    // Add size class
    if (size !== 'default') {
      classes.push(styles[`inputNumberSize${size.charAt(0).toUpperCase() + size.slice(1)}`]);
    }
      ...
      <div className={getWrapperClass()}>
        ...
        <div className={styles.inputNumberControls}>
          <button
            ...
            disabled={disabled || readOnly || (max !== undefined && (internalValue || 0) >= max)}
          ...
          <button
            type="button"
            className={styles.inputNumberButton}
            disabled={disabled || readOnly || (min !== undefined && (internalValue || 0) <= min)}
