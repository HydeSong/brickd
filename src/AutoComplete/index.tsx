...
interface AutoCompleteProps {
  ...
  loading?: boolean;
  filterOption?: (inputValue: string, option: AutoCompleteOption | string) => boolean;
  renderOption?: (option: AutoCompleteOption | string, inputValue: string) => React.ReactNode;
  className?: string;
...
export const AutoComplete: React.FC<AutoCompleteProps> = ({
  ...
  const [filteredOptions, setFilteredOptions] = useState<(AutoCompleteOption | string)[]>([]);
    ...
    if (internalValue && options.length > 0) {
      const filtered = options.filter(option => {
        ...
        if (typeof option === 'string') {
          ...
          return option.label?.toLowerCase().includes(internalValue.toLowerCase()) || 
  ...
  const getInputClass = () => {
    ...
    // Add size class
    if (size !== 'default') {
      classes.push(styles[`autoCompleteSize${size.charAt(0).toUpperCase() + size.slice(1)}`]);
    }
  ...
  const renderOptionItem = (option: AutoCompleteOption | string, index: number) => {
