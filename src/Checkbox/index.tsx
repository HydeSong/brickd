...
export const Checkbox: CheckboxComponent = ({
  ...
  const getCheckboxClass = () => {
    ...
    // Add size class
    if (size !== 'default') {
      classes.push(styles[`checkboxSize${size.charAt(0).toUpperCase() + size.slice(1)}`]);
    }
...
const CheckboxGroup: React.FC<CheckboxGroupProps> = ({ 
  ...
  const handleOptionChange = (optionValue: string | number | boolean, checked: boolean) => {
  ...
  const getGroupClass = () => {
    ...
    // Add direction class
    classes.push(styles[`checkboxGroup${direction.charAt(0).toUpperCase() + direction.slice(1)}`]);
