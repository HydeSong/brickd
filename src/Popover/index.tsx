...
const Popover: React.FC<PopoverProps> = ({ 
  ...
  const getPlacementClass = () => {
    return styles[`popover${placement.charAt(0).toUpperCase() + placement.slice(1)}`];
  };
    ...
    <div className={`${styles.popoverWrapper} ${className}`} style={style}>
      ...
      {internalVisible && (
        ...
        >
          ...
          <div className={`${styles.popoverArrow} ${styles[`arrow${placement.charAt(0).toUpperCase() + placement.slice(1)}`]}`} />
        </div>
      )}
    </div>
  );
