...
export const Transfer: React.FC<TransferProps> = ({
  ...
  const getFilteredSourceList = () => {
    ...
    return sourceList.filter((item) =>
      (item.label as string)
        .toLowerCase()
        .includes(sourceSearchValue.toLowerCase()),
    );
  };
  ...
  const handleSourceSelectAll = (checked: boolean) => {
    ...
    const enabledKeys = filteredSourceList
  ...
  const handleTargetSelectAll = (checked: boolean) => {
    ...
    const enabledKeys = filteredTargetList
      .filter((item) => !item.disabled)
      .map((item) => item.key);
  ...
  const handleMoveLeft = () => {
    const newInternalValue = internalValue.filter(
  ...
  const renderList = (title: string, isSource: boolean) => {
    ...
    const selectedEnabledItems = enabledItems.filter((item) =>
  ...
  return (
    <div
      className={`${styles.transfer} ${
        disabled ? styles.transferDisabled : ''
      } ${className}`}
      style={style}
    >
      ...
      <div className={styles.transferOperations}>
        <button
          ...
          className={`${styles.transferOperationButton} ${
            canMoveRight
              ? styles.transferOperationButtonPrimary
              : styles.transferOperationButtonDisabled
          }`}
        ...
        <button
          ...
          disabled={
            disabled ||
            getFilteredSourceList().filter((item) => !item.disabled).length ===
              0
          }
        >
          ⇒
        ...
        <button
          ...
          disabled={
            disabled ||
            getFilteredTargetList().filter((item) => !item.disabled).length ===
              0
          }
        >
          ⇐
