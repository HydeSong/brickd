...
export const Slider: React.FC<SliderProps> = ({
  ...
  const getSliderClass = () => {
    ...
    if (size !== 'default') {
      classes.push(
        styles[`sliderSize${size.charAt(0).toUpperCase() + size.slice(1)}`],
      );
    }
          ...
          return (
            <div
              ...
              className={`${styles.sliderMark} ${
                isActive ? styles.sliderMarkActive : ''
              }`}
              style={markStyle}
            >
              {mark.label}
  ...
  const renderTooltip = () => {
    const shouldShowTooltip =
      tooltipVisible !== undefined ? tooltipVisible : tooltip && tooltipOpen;
    if (!shouldShowTooltip) return null;

    return <div className={styles.sliderTooltip}>{internalValue}</div>;
  };
